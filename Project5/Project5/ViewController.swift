...
//  Project5
//
//  Created by Maris Lagzdins on 06/09/2019.
...
class ViewController: UITableViewController {
    ...
    func continuePreviousGame() -> Bool {
        let defaults = UserDefaults.standard
        return defaults.object(forKey: "CurrentWord") != nil
    }
        ...
        if isPossible(word: lowerAnswer) {
            if isOriginal(word: lowerAnswer) {
                if isReal(word: lowerAnswer) {
                    ...
                    showErrorMessage(errorMessage, title: errorTitle)
                }
            } else {
    ...
    func isReal(word: String) -> Bool {
        ...
        guard let title = title, word != title else { return false }
